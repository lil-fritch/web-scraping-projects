...
agent = {}
# for page in Page.objects.filter(status='New').order_by('id'):
def main(page):
    respone = requests.get(page.link, headers=headers, cookies=cookies, allow_redirects=False)
    if respone.status_code == 200:
        ...
        if not agent['url'] or not agent['phone']: 
            page.status = 'Done'
            page.save()
            return
