'''
Searches the site with all possible combinations of 3 letters 
and saves all available information on the search page
'''
    ...
    for letter2 in alphabet:
        for letter3 in alphabet:
            for page in range(1, 100):
                print(f'Checking {letter1}{letter2}{letter3}')
                respone = requests.get(
                    f'https://www.argenprop.com/inmobiliarias?Nombre={letter1}{letter2}{letter3}&Orden=CantidadAvisos&pagina={page}',
                    headers=headers,
                    cookies=cookies
                    )
                ...
                if respone.status_code == 200:
                    ...
                    if 'Parece que no hay inmobiliarias que coincidan con el nombre ingresado.' in respone.text:
                        break
                    ...
                    for agent_data in agents_data:
                        try:
                            link_element = agent_data.find_parent()
                            if link_element:
                                link = link_element.get('href')
                                if link:
                                    agent['url'] = f'https://www.argenprop.com/{link}'
                            else:
                                agent['url'] = None
                        ...
                        try:
                            ...
                            if agent['image'] == '/content/images/no-agent-logo.svg':
                                agent['image'] = None
                        except AttributeError:
                            agent['image'] = None   
